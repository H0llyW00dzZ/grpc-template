...
- `client.WithLoadBalancing(policy)` configures client-side load balancing. Registered policies: `"pick_first"` (default), `"round_robin"`, `"weighted_round_robin"`, `"least_request_experimental"`, `"ring_hash_experimental"`. The policy is validated against the gRPC balancer registry; unknown policies defer an error to `Connect()`. Use `dns:///` target prefix for multi-endpoint resolution.
