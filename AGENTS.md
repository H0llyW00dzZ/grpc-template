...
- `client.WithLoadBalancing(policy)` configures client-side load balancing (e.g., `"round_robin"`). The policy is validated against the gRPC balancer registry; unknown policies defer an error to `Connect()`. Use `dns:///` target prefix for multi-endpoint resolution.
