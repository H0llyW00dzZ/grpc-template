...
- `server.WithDefaultServiceConfig(json)` advertises a default gRPC service config (e.g., load balancing policy) to clients via name resolvers. The `Server.ServiceConfig()` accessor returns the configured JSON string.
- `client.WithLoadBalancing(policy)` configures client-side load balancing. Policies must be registered via `import _ "internal/client/balancer"` (registers all built-in policies) or by importing individual upstream balancer packages. Registered policies: `"pick_first"` (default), `"round_robin"`, `"weighted_round_robin"`, `"least_request_experimental"`, `"ring_hash_experimental"`. The policy is validated against the gRPC balancer registry; unknown policies defer an error to `Connect()`. Use `dns:///` target prefix for multi-endpoint resolution.
- `RequestID` / `StreamRequestID` interceptors validate incoming `x-request-id` values against strict UUID format (8-4-4-4-12 hex). Non-matching or missing values are replaced with a server-generated UUID to prevent spoofing and log injection.
